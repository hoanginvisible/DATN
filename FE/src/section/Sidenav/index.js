...
function Sidenav({roleUrl, color, brand, brandName, routes, ...rest }) {
    ...
    ({ type, name, icon, title, key, href, route }) => {
      ...
      if (type === "route") {
        if (href) {
          ...
          returnValue = (
            <NavLink to={roleUrl + route} key={key}>
              <SidenavItem name={name} icon={icon} active={key === itemName} />
            </NavLink>
          );
        }
  ...
  return (
    ...
    >
      <ArgonBox pt={3} pb={1} px={0} textAlign="center">
        <ArgonBox
        ...
        >
          <ArgonTypography variant="h6" color="secondary">
            <CloseIcon sx={{ fontWeight: "bold" }}>close</CloseIcon>
          </ArgonTypography>
        </ArgonBox>
